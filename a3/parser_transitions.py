...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        print('=====')
        print(transitions)
        for (transition, parseline) in zip(transitions, minibatch):
            parseline.parse_step(transition)
            if len(parseline.stack) == 1 and not parseline.buffer:
                unfinished_parses.remove(parseline)
    dependencies = [parseline.dependencies for parseline in partial_parses]
    ### END YOUR CODE
