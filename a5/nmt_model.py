...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        #print(source_padded_chars.shape)
        #print(source_padded_chars)
        #print(target_padded_chars.shape)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, lens_unpacked = pad_packed_sequence(enc_hiddens, batch_first=True)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # print("ybar size")
        # print(Ybar_t.shape)
        dec_state_new = self.decoder(Ybar_t, dec_state)
        dec_hidden, dec_cell = dec_state_new
        # print("hidden state and cell size")
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        Msoftmax = nn.Softmax(1)
        alpha_t = Msoftmax(e_t)
        #print(alpha_t.size())
        #print(alpha_t.shape)
        #print("test")
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            if len(decoderStatesForUNKsHere) > 0 and self.charDecoder is not None: # decode UNKs
                ...
                #print("input decode greedy")
                #print(decoderStatesForUNKsHere.shape)
                #print(decoderStatesForUNKsHere)
                #print(decoderStatesForUNKsHere.unsqueeze(0).shape)
