...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        #self.start_of_word
        #self.end_of_word
        return [[[self.start_of_word]+[self.char2id[unitchard] for unitchard in w]+[self.end_of_word] for w in s] for s in sents]
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        word_t = pad_sents_char(word_ids, self.char2id['<pad>'])
        #word_t = pad_sents_char(word_ids, self['<pad>'])
        word_var = torch.tensor(word_t, dtype=torch.long, device=device)
        return torch.transpose(word_var, 0, 1)
        ### END YOUR CODE
